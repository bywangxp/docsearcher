...
var l=opts.leng;
  ...
  function getTableInfo(p) {
    alert(p);
    $.ajax({
      url: '/DocSearcher/user/collect_data',
      type: 'POST',
      data: 'page=' + p,
      dataType: 'json',
      success: function(json0) {
        ...
        for (var i = 0; i < oRow.length; i++) {
          rowData = '<li class="row clearfix">' + '<div class="n1">' + oRow[i].logo + '</div><div class="n2">' + oRow[i].docname + '</div><div class="n3">' + oRow[i].clc_time + '</div><div class="n4">' + oRow[i].download + '</div><div class="n5">' + oRow[i].credit + '</div><div class="n6">' + oRow[i].score + '</div>' + '</li>' + rowData;
          Id.push(oRow[i].id);
        }
        $('#tRow').html(rowData);
      },
